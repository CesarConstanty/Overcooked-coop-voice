...
class Mlam_generator:
    ...
    def gen_mlam(self, layout_name):
        mdp = OvercookedGridworld.from_layout_name(
            layout_name, self.config.get("layouts_dir", "overcooked_ai_py/data/layouts")
        )
        ...
        mlam = MediumLevelActionManager.from_pickle_or_compute(mdp, counter_params, force_compute=False, info=True)
        ...
        if self.config["copy"]:
            ...
            motion_planner.save_to_file(
                "/tmp_user/ldtis960b/mleguill/pickles/" + os.path.join(self.config["name"], layout_name + "_mp.pkl")
            )
            mlam.save_to_file(
                "/tmp_user/ldtis960b/mleguill/pickles/" + os.path.join(self.config["name"], layout_name + "_am.pkl")
            )
