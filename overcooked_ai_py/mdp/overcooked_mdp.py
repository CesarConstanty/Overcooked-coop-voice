...
class OvercookedGridworld(object):
    ...
    def get_random_start_state_fn(self, random_start_pos=False, rnd_obj_prob_thresh=0.0):
        def start_state_fn():
            ...
            start_state = OvercookedState.from_player_positions(
                start_pos, bonus_orders=self.start_bonus_orders, all_orders=self.start_all_orders
            )
    ...
    def featurize_state(self, overcooked_state, mlam, num_pots=2, **kwargs):
        ...
        for i, player in enumerate(overcooked_state.players):
            ...
            all_features = concat_dicts(
                all_features,
                make_closest_feature(i, player, "empty_counter", self.get_empty_counter_locations(overcooked_state)),
            )
    ...
    def potential_function(self, state, mp, gamma=0.99):
        ...
        opt_recipe, discounted_opt_recipe_value = self.get_optimal_possible_recipe(
            state, None, discounted=True, potential_params=potential_params, return_value=True
        )
        ...
        non_idle_soup_vals = {
            ...
            ** (
                ...
                + max(potential_params["max_pickup_steps"], soup.cook_time - soup._cooking_tick)
            )
